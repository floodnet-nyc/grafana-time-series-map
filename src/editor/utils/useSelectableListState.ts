...
export function useSelectableListState<T>({ items, onChange, removeBehavior = 'preserve' }: Options<T>) {
  const [selectedIndex, setSelectedIndex] = useState<number | null>(null);
  const visibleSelectedIndex = selectedIndex !== null && selectedIndex < items.length ? selectedIndex : null;
